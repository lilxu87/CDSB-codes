...
def conditional_imputation_eval_mdp(
        impute_function,
        opt,
        test_queue,
        run,
    ...
    for batch_id, batch in enumerate(test_queue):
        ...
        samples = samples.cpu()
        c_target = observed_data.cpu()
        eval_points = target_mask.cpu()
        observed_points = obs_mask.cpu()

        all_generated_samples.append(samples.float().unsqueeze(dim=0))
        all_target.append(c_target.unsqueeze(dim=0))
        all_evalpoint.append(eval_points.unsqueeze(dim=0))
        all_observed_point.append(observed_points.unsqueeze(dim=0))
