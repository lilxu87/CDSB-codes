...
class BaseSDE(metaclass=abc.ABCMeta):
    ...
    def sample_traj(
        ...
        if opt.train_method=='joint':
            ...
            _ts = tqdm(ts, ncols=80, file=sys.stdout, desc=f"{direction} SDE sampling...")
    ...
    def sample_traj_conditional(
        ...
        if opt.train_method=='joint':
            ...
            _ts = tqdm(ts, file=sys.stdout, ncols=80, desc="SDE sampling...")
        for idx, t in enumerate(_ts):
            _t=t if idx==ts.shape[0]-1 else ts[idx+1]
    ...
    def sample_traj_imputation_forward(
        ...
        if opt.train_method=='joint':
            ...
            _ts = tqdm(ts, file=sys.stdout, ncols=80, desc="Forward SDE sampling...")
        for idx, t in enumerate(_ts):
            _t=t if idx==ts.shape[0]-1 else ts[idx+1]
    ...
    def compute_nll(self, samp_bs, ts, z_f, z_b):
        ...
        for idx, t in enumerate(tqdm(ts,desc="Propagating Dynamics...")):
