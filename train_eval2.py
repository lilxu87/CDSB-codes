...
flags.DEFINE_string('env_name', 'Walker2d-v2',
...
flags.DEFINE_float('target_policy_std', 0, 'Noise scale of target policy.')
...
flags.DEFINE_float('discount', 1, 'Discount used for returns.')
...
flags.DEFINE_string(
    'data_dir',
    '/home/caohaoqun/CSBI_replicant/policy_eval/trajectory_datasets/',
...
def main(_):
  ...
  if FLAGS.d4rl:
    ...
    actor.load_weights(os.getcwd()+"/"+behavior_dataset.model_filename)

  # utils.sample_dynamic(FLAGS.env_name, FLAGS.seed, actor,
  #                                       std=0, num_samples = 100)
  # dataset, empirical_return = utils.collect_stochastic_env_dataset(env, actor, std = 0,
  #                                               num_episodes = FLAGS.num_trajectories, horizon = 50, max_length = 1000)
  ...
  # DSB_return = utils.estimate_returns(env, actor, 0, FLAGS.num_trajectories, initial_states = dataset.initial_states, DSB = run, opt = opt)

  # print("bias is ", empirical_return - DSB_return)

  # policy_returns = empirical_return
  # policy_returns = utils.estimate_monte_carlo_returns(env, FLAGS.discount,
  #                                                     actor,
  #                                                     FLAGS.target_policy_std,
  #                                                     FLAGS.num_mc_episodes)
  
  # policy_returns = 0.349618
  ...
  # min_state = np.min(dataset.states, 0)
  # max_state = np.max(dataset.states, 0)
